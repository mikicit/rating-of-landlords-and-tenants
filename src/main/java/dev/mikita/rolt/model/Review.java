...
@NamedQuery(
        name = "Review.findAverageRatingByConsumer",
        query = "SELECT AVG(r.rating) FROM Review r " +
                "WHERE (r.contract.tenant = :consumer " +
                "OR r.contract.property.owner = :consumer) " +
                "AND r.author <> :consumer")
public class Review implements Serializable {
    ...
    public void setUpdatedOn(LocalDateTime updatedOn) {
        ...
        if (updatedOn.isBefore(createdOn)) {
            throw new IllegalStateException("The update date must be after the creation date.");
        }
    ...
    public void setRating(Integer rating) {
        ...
        if (rating < 1 || rating > 5) {
            throw new IllegalStateException("The rating value should be between 1 and 5.");
        }
    ...
    public void prePersist() {
        if (!author.equals(contract.getTenant()) && !author.equals(contract.getProperty().getOwner())) {
            throw new IllegalStateException("The user has no right to leave feedback for this contract");
        }
    }
