...
public class TenantDao extends BaseDao<Tenant> {
    ...
    public List<Tenant> findAllInSearch() {
        try {
            return em.createQuery("SELECT t FROM Tenant t WHERE t.details.status = dev.mikita.rolt.entity.ConsumerStatus.ACTIVE AND t.inSearch = true", Tenant.class).getResultList();
        } catch (RuntimeException e) {
            throw new PersistenceException(e);
        }
    }
